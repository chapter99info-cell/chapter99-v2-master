...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const body = req.body as Partial<OwnerBookingNotificationPayload>
  if (!body.to?.trim()) {
  ...
  const payload: OwnerBookingNotificationPayload = {
    to: body.to.trim(),
    ...
    date: body.date?.trim() || '',
    time: body.time?.trim() || '',
    therapistLabel: body.therapistLabel?.trim(),
  ...
  try {
    ...
    const result = await resend.emails.send({
      from: FROM,
      to: payload.to,
      subject: buildOwnerBookingNotificationSubject(payload.shopName, payload.clientName),
      html: buildOwnerBookingNotificationHTML(payload),
      text: buildOwnerBookingNotificationText(payload),
    })

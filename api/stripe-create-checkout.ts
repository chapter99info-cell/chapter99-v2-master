...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!secret) {
    return sendJsonError(
      res,
      500,
      'STRIPE_SECRET_KEY is not configured on the server (Vercel → Settings → Environment Variables)'
    )
  }
  ...
  const shopQuery = body.shopSlug?.trim()
    ? `&shop=${encodeURIComponent(body.shopSlug.trim().toLowerCase())}`
    : ''
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${origin}/voucher?success=1&session_id={CHECKOUT_SESSION_ID}${shopQuery}`,
    cancel_url: `${origin}/voucher?cancelled=1${shopQuery}`,
  })
